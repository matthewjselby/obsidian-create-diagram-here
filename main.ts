...
const newSVGText = `<?xml version="1.0" encoding="UTF-8" standalone="no"?>
...
export default class CreateImageHerePlugin extends Plugin {
	...
	async createAndOpenImage() {
		if (this.app.workspace.activeEditor?.editor != null) {
			...
			await this.app.vault.create(pathForSVG, newSVGText);
	...
	async onload() {
		...
		this.registerEvent(
			this.app.workspace.on("file-menu", (menu, file) => {
				menu.addItem((item) => {
					item
						.setTitle("Open image for editing 📝")
						.setIcon("document")
...
class CreateImageHereSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.setDesc('Absolute path to desired image editor on your computer. If left empty, the default image editor will be used.')
			.addText(text => text
				.setPlaceholder('')
